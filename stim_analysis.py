...
def main():
	...
	2. Matching for SuperLab
	3. RTs - 250 
	4. Look into Clapp data
	"""

	#subject's data
	subject_data = list(csv.reader(open('/Users/almaskebekbayev/Desktop/raw_data/000JasmineOhadi.txt','rb'), delimiter='\t'))
	#master data
	master_test = DataFrame(pd.read_csv('/Users/almaskebekbayev/Dropbox/Exp 1 Face Fan Data/stim/s000/master-test.csv'))
	...
	#NOTE: subtract 250 from the time values
	#for i df_data['time'].values.tolist():
	...
	a = [int(x)-250 for x in df_data['time'].values.tolist()]
	...
	#out = csv.writer(open('010MollyFontaine_csv.txt', 'w'), delimiter=',')
